...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.name.call(), 'MyNonFungibleToken');
    assert.equal(await instance.symbol.call(), "NFT");
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let tokenId = 101;
    let instance = await StarNotary.deployed();
    await instance.createStar('MyShouldEqualStar', tokenId, {from: accounts[0]});
    assert.equal(await instance.lookUptokenIdToStarInfo.call(tokenId), 'MyShouldEqualStar');
});